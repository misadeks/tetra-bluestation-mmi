...
use crate::{ContactRow, ConvRow, DialTarget, EntityRow, FolderRow, FormField, LogRow, MainWindow, MsgRow, PickRow, RecentRow, ScanRow, Screen, SurveyRow, TreeRow};
...
/// state for selection). A non-empty search query filters folders and groups,
/// auto-expanding folders that contain matches.
...
/// Build the Scan lists model (each programmed scan list with its active state).
fn push_scanlists(app: &AppState, weak: &slint::Weak<MainWindow>) {
